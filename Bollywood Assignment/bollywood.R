...
#	top_movie <- bollywood[which.max(bollywood$Tcollection), ]$Movie
	...
	top_movie <- bollywood[which.max(sapply(bollywood$Tcollection,max)), ]$Movie 
  top_movie
...
# second_top <- bollywood[-c(which.max(bollywood$Tcollection)), ]
#  second_top
#  top_2_movie <- second_top[which.max(second_top$Tcollection), ]$Movie
#  top_2_movie
...
#  shahrukh_collection <- apply(shahrukh["Tcollection"],2,sum)
	
	shahrukh_collection <- sapply(shahrukh["Tcollection"],sum)
  shahrukh_collection
	akshay_collection <- sapply(akshay["Tcollection"],sum)
	akshay_collection
	amitabh_collection <- sapply(amitabh["Tcollection"],sum)
	amitabh_collection
...
# movie_result
# movie_result <- bollywood[c(which.max(sapply(bollywood$Ocollection, max)),which.max(sapply(bollywood$Wcollection, max)),which.max(sapply(bollywood$Fwcollection, max)),which.max(sapply(bollywood$Tcollection, max))), ]$Movie
# movie_result  
